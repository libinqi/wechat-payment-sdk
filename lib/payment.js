...
Payment.prototype.getBarCodeWCPayRequestParams = function (order, callback) {
  var self = this;
  var default_params = {
    appId: this.appId,
    timeStamp: this._generateTimeStamp(),
  ...
  this.microPay(order, function (err, data) {
    if (err) {
      return callback(err);
    }

    var params = _.extend(default_params, data);
...
Payment.prototype.microPay = function (params, callback) {
  var requiredData = ['body', 'out_trade_no', 'total_fee', 'spbill_create_ip', 'auth_code'];
...
Payment.prototype.reverseOrder = function (params, callback) {
...
Payment.prototype.validate = function (xml, callback) {
  ...
  xml2js.parseString(xml, {
    ...
    if (data.return_code == RETURN_CODES.FAIL) {
      ...
      data.err_code != "USERPAYING" &&
      data.err_code != "SYSTEMERROR") {
